...
int                         gRenderToTextureNumber=0;
	...
	//
	// Bunch of legacy definitions, never used but put here for compatibility.
	// (Remember, I found that GL isn't emulated in WASM when it comes to 3d and lights n stuff, so 
	// I moved to GL3)
	//
	void 						DisableLights(); // Legacy
	void 						SetAmbientLight(float r, float g, float b); // Legacy
	void 						SetMaterial(float ambientR,float ambientG,float ambientB, float ambientA, float diffuseR,float diffuseG,float diffuseB,float diffuseA, float specR,float specG,float specB, float specA, float emR,float emG,float emB, float emA, float thePower); // Legacy
	void 						AddOmniLight(float x, float y, float z, float theRange,float r, float g, float b, float sr, float sg, float sb, float constAtten, float inverseAtten,float squareAtten, float fallOff); // Legacy
	void 						AddSkyLight(float xdir, float ydir, float zdir, float r, float g, float b, float sr, float sg, float sb); // Legacy
	void 						DrawLine(void *theBuffer, int theLineCount, int theVertexStride);
	int							CreateShader(char* theVertexShader, char* thePixelShader, Array<char*>& theVertexFormat) {return -1;}
	void						SetShader(int theShader) {}
	int							GetShaderGlobalRef(int theShader, int vertexOrPixel, char* theVariableName) {return -1;}
	...
	int							CreateShape(int theShader, void* theVertexList, int theVertexCount, short* theIndexList, int theIndexCount) {return -1;}
...
void Graphics_Core::Startup_SDL()
{
	...
	//OS_Core::Error("OpenGL version supported by this platform : %s", glGetString(GL_VERSION));
	...
	if (!gSDLScreen) OS_Core::Error("Could not create SDL Screen: %s",SDL_GetError());
...
{
	if (strcasecmp(theQuery,"NoShaders")==0) return (void*)1; // Kludge because our standard loaded images come in correctly for Android... but custom formats like ".thumb" won't!
...
namespace Graphics_Core
{
	...
	void CreateGLTexture(int theWidth, int theHeight, int theFormat, char *theBits)
	{
		...
		//
		/*
		char* aBit=theBits;for (int aCount=0;aCount<theWidth*theHeight;aCount++) {*aBit=255;aBit++;*aBit=0;aBit++;*aBit=0;aBit++;*aBit=255;aBit++;}
		*/
...
void Graphics_Core::SetTexture(int theNumber, char* specialOp)   // P0dprime: match graphics_core.h
{
	//
